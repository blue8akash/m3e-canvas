...
**Removed 2026-08-24** (thin stub, no real content): `plugins/` (packaging — add back only when this agent system actually needs installing across multiple repos). `hooks/` was kept — it ships a real check (`post-task-orientation-update.md`), unlike the equivalent folder some individual repos removed as an empty stub.
